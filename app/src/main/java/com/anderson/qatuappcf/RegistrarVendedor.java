...
// Establecer el filtro de longitud máximo a 10 caracteres
        ...
        btnVolver.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent i = new Intent(RegistrarVendedor.this, AdminVendedor.class);
                startActivity(i);
                finish();
            }
        });
    ...
    public void Guardar() {
        ...
        if (TextUtils.isEmpty(cedula) || !validarCedula(cedula)) {
            Toast.makeText(RegistrarVendedor.this, "Cédula ecuatoriana inválida", Toast.LENGTH_SHORT).show();
            return;
        }

        if (TextUtils.isEmpty(email)) {
            Toast.makeText(RegistrarVendedor.this, "Ingresar Mail", Toast.LENGTH_SHORT).show();
            return;
        }
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            Toast.makeText(RegistrarVendedor.this, "Correo electrónico inválido", Toast.LENGTH_SHORT).show();
            return;
        }

        if (TextUtils.isEmpty(pass)) {
            Toast.makeText(RegistrarVendedor.this, "Ingresar Password", Toast.LENGTH_SHORT).show();
            return;
        }
        if (pass.length() < 6) {
            Toast.makeText(RegistrarVendedor.this, "La contraseña debe tener al menos 6 caracteres", Toast.LENGTH_SHORT).show();
            return;
        }
        if (!pass.equals(conPass)) {
            Toast.makeText(RegistrarVendedor.this, "Las contraseñas no coinciden", Toast.LENGTH_SHORT).show();
            return;
        }
                    ...
                    public void onComplete(@NonNull Task<SignInMethodQueryResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (result != null && result.getSignInMethods() != null && result.getSignInMethods().size() > 0) {
                                ...
                                Toast.makeText(RegistrarVendedor.this, "El correo electrónico ya está registrado", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                personaRef.orderByChild("cedula").equalTo(cedula).addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                        if (dataSnapshot.exists()) {
                                            ...
                                            Toast.makeText(RegistrarVendedor.this, "La cedula ya está registrada", Toast.LENGTH_SHORT).show();
                                        } else {
                                                    ...
                                                    .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                                                        ...
                                                        public void onComplete(@NonNull Task<AuthResult> task) {
                                                            if (task.isSuccessful()) {
                                                                ...
                                                                newUserRef.child("tipo").setValue("vendedor");
                                                                newUserRef.child("email").setValue(email)
                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                            @Override
                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                if (task.isSuccessful()) {
                                                                                    Toast.makeText(RegistrarVendedor.this, "Registrado correctamente", Toast.LENGTH_SHORT).show();
                                                                                    limpiar();
                                                                                } else {
                                                                                    Toast.makeText(RegistrarVendedor.this, "Fallo al guardar la información en la base de datos", Toast.LENGTH_SHORT).show();
                                                                                }
                                                                            }
                                                                        });
                                                            } else {
                                                                Toast.makeText(RegistrarVendedor.this, "Fallo en Autenticación", Toast.LENGTH_SHORT).show();
                                                            }
                                                        }
                                                    });
                                        }
                            ...
                            Toast.makeText(RegistrarVendedor.this, "Error al verificar el correo electrónico", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
